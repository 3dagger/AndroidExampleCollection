package com.dagger.navermapclustering.clustering.view
...
internal class ClusterRenderer<Clustering, T : LeeamClusterItem, RealMarker, Marker : LeeamMarker<ImageDescriptor>, Map, ImageDescriptor>(
	...
	private val leeamMap: LeeamMap<RealMarker, Marker, ImageDescriptor> = builder.map
		...
		mClusterManager.markerMarkerCollection
			.setOnMarkerClickListener { marker ->
				if (builder.clickToCenter) {
					leeamMap.moveToCenter(marker.position)
				}
	...
	private inner class ViewModifier : Handler() {
		...
		override fun handleMessage(msg: Message) {
			...
			renderTask.setMapZoom(leeamMap.getCameraPosition().zoom)
	 ...
	 *
	 *
	 * 1. Markers are added to the leeamMap
	 *
	 *
	 * 2. Markers are animated to their final position
	 *
	 *
	 * 3. Any old markers are removed from the leeamMap
	 *
	 *
	 ...
	 */
	private inner class RenderTask(internal val clusters: Set<Cluster<T>>) : Runnable {
		...
		override fun run() {
			...
			val visibleBounds: LeeamLatLngBounds = try {
				leeamMap.getVisibleLatLngBounds()
			} catch (e: Exception) {
				e.printStackTrace()
				LeeamLatLngBounds()
			}
	...
	/**
	 * Handles all markerWithPosition manipulations on the leeamMap. Work (such as adding, removing, or
	 ...
	 */
		...
		fun perform(markerModifier: MarkerModifier) {
			...
			if (!shouldRenderAsCluster(cluster)) {
				for (item in cluster.items) {
					...
					if (marker == null) {
						...
						leeamMap.addMarkerClickListener(markerOptions, { marker ->
			...
			val markerWithPosition: MarkerWithPosition<Marker, ImageDescriptor>
			if (marker == null) {
				val defaultMarker = leeamMap.getMarker()
			...
			newMarkers.add(markerWithPosition)
			leeamMap.addMarkerClickListener(marker, { marker ->
				mClusterManager.onMarkerClick(marker)
			})
		}
	}
